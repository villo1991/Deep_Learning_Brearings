...
def kurtosis_pool(x):
    ...
    return fourth / (tf.pow(var, 2) + 1e-6)
...
def ResNet1D(input_length, num_classes, useBN=True):
    ...
    x = ResidualBlock1D(8, 31, dilation_rate=2)(x)  # 1
    x = ResidualBlock1D(8, 31, downsample=True)(x)  # 2
    ...
    x = ResidualBlock1D(16, 21, downsample=True)(x) # 8
    x = ResidualBlock1D(16, 15, dilation_rate=4)(x) # 9
    x = ResidualBlock1D(16, 7, downsample=True)(x) # 10
    #oichi neuroni perchè hanno una vista incredibile

    # --- HEAD (per segnali impulsivi)
    #kurt_pool = Lambda(kurtosis_pool)(x)
...
if __name__ == "__main__":
    ...
    base_path = "......./paderborn_university/dataset/10024/"
    X_train_path = base_path + "X_train_raw_10024_aug_stdV1.npy"
    y_train_path = base_path + "Y_train_raw_10024_aug_ohe.npy"
    X_test_path  = base_path + "X_test_raw_10024_noaug_stdV1.npy"
    y_test_path  = base_path + "Y_test_raw_10024_noaug_ohe.npy"
    ...
    checkpoint_path = os.path.join("......./paderborn_university/results/10024", outdir, "best_model.keras")
    ...
    abs_path = os.path.join("/home/villo/projects/bearings_project/paderborn_university/results/10024", outdir)
